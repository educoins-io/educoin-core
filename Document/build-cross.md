Cross-compiliation of Educoin Core
===============================

Educoin Core can be cross-compiled on Linux to all other supported host systems. This is done by changing
the `HOST` parameter when building the dependencies and then specifying another `--prefix` directory when building Educoin.
...
When building Educoin Core, use
...
Note that for WSL the Educoin Core source path MUST be somewhere in the default mount file system, for
...
It's not possible to build the Educoin Core code using the win32 version of the Mingw-w64 cross compilers (at least not without
modifying headers in the Educoin Core source code).
