...
- [Development guidelines](#development-guidelines)
    - [General Educoin Core](#general-educoin-core)
    - [Wallet](#wallet)
    - [General C++](#general-c)
...
Educoin Core is a multi-threaded application, and deadlocks or other
...
([`valgrind.supp`](https://github.com/educoinpay/educoin/blob/master/contrib/valgrind.supp))
which includes known Valgrind warnings in our dependencies that cannot be fixed
in-tree. Example use:

```shell
$ valgrind --suppressions=contrib/valgrind.supp src/test/test_educoin
$ valgrind --suppressions=contrib/valgrind.supp --leak-check=full \
      --show-leak-kinds=all src/test/test_educoin --log_level=test_suite
$ valgrind -v --leak-check=full src/educoind -printtoconsole
```
...
# A coverage report will now be accessible at `./test_educoin.coverage/index.html`.
```

**Sanitizers**

Educoin Core can be compiled with various "sanitizers" enabled, which add
...
- ThreadSocketHandler : Sends/Receives data from peers on port 38110.
...
- ThreadRPCServer : Remote procedure call handler, listens on port 38109 for connections and services them.
...
- Try not to overload methods on argument type. E.g. don't make `getblock(true)` and `getblock("hash")`
  ...
  - *Rationale*: This is impossible to use with `educoin-cli`, and can be surprising to users.
...
- Add every non-string RPC argument `(method, idx, name)` to the table `vRPCConvertParams` in `rpc/client.cpp`.
  ...
  - *Rationale*: `educoin-cli` and the GUI debug console use this table to determine how to
...
- Wallet RPCs call BlockUntilSyncedToCurrentChain to maintain consistency with
  ...
  - *Rationale*: In previous versions of Educoin Core, the wallet was always
